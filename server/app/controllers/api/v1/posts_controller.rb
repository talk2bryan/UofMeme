...
content_type = Mime::Type.lookup_by_extension(File.extname(params[:post]
[:file_name])[1..-1]).to_s
image_file = Paperclip.io_adapters.for("data:#{content_type};base64,#{params[
:post][:uploaded_image_for_io_adapters]}")
...
def post_params
params.permit(:poster, :description, :user_id, :file_name, :uploaded_image_for_
io_adapters)
end
